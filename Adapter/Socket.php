...
class Socket extends AbstractAdapter
{
    ...
    private function connect($config)
    {
        ...
        if (is_array($this->proxyList)) {
            ...
            $errno = $errstr = null;
            $this->sock = @stream_socket_client('tcp://' . $config['server'] . ':' . $config['port'], $errno, $errstr, 2);
        }
    ...
    private function connectToProxy(array $proxyConfig, array $config)
    {
        ...
        $socket = @stream_socket_client($proxyScheme . $proxyHost, $errno, $errstr, 2);
    ...
    private function getRandomProxy()
    {
        ...
        while ($i--) {
            ...
            if (
                isset($proxyConfig['enabled']) && $proxyConfig['enabled'] == 1 && !empty($proxyConfig['port'])
